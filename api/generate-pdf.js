...
export default async function handler(req, res) {
  ...
  try {
    ...
    logRequest('pdf-generated', { auditId, size: pdfBuffer.length, url: blob.url });
    ...
    <Document>
      ...
      <Page size="A4" style={styles.page}>
        <View style={styles.coverPage}>
          ...
          <Text style={styles.performanceLevel}>{auditData.performanceLevel}</Text>
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.section}>
          ...
          <Text style={styles.paragraph}>{auditData.executiveSummary.currentState}</Text>

          <Text style={styles.subheading}>Key Findings</Text>
          {auditData.executiveSummary.keyFindings.map((finding, index) => (
            <Text key={index} style={styles.bulletPoint}>• {finding}</Text>
          ))}

          <Text style={styles.subheading}>Potential Impact</Text>
          <Text style={styles.paragraph}>{auditData.executiveSummary.potentialImpact}</Text>
        </View>
      </Page>
        ...
        {auditData.criticalIssues.map((issue, index) => (
          <View key={index} style={styles.issueCard}>
            <View style={styles.issueHeader}>
              <Text style={styles.issueTitle}>{index + 1}. {issue.title}</Text>
          ...
          {Object.entries(auditData.sectionScores).map(([key, section]) => (
            <View key={key} style={styles.scoreRow}>
              ...
              <View style={styles.scoreBar}>
                <View style={[styles.scoreBarFill, { width: `${section.score}%` }]} />
                <Text style={styles.scoreBarText}>{section.score}/100</Text>
              </View>
              <Text style={[styles.statusBadge, getStatusStyle(section.status)]}>
                {section.status}
              </Text>
              <Text style={styles.priorityText}>{section.priority} Priority</Text>
            </View>
          ))}
        </View>
      </Page>
      ...
      <Page size="A4" style={styles.page}>
        ...
        <Text style={styles.subtitle}>Easy improvements you can make today</Text>
        {auditData.quickWins.map((win, index) => (
          <View key={index} style={styles.quickWinCard}>
            ...
            <View style={styles.quickWinMeta}>
              ...
              <Text style={styles.quickWinImpact}>Impact: {win.expectedImpact}</Text>
            </View>
        ...
        <View style={styles.section}>
          <View style={styles.revenueGrid}>
            <View style={styles.revenueBox}>
              ...
              <Text style={styles.revenueValue}>{auditData.revenueOpportunity.currentEstimate}</Text>
            </View>
            <View style={styles.revenueBox}>
              <Text style={styles.revenueLabel}>Optimized Estimate</Text>
              <Text style={styles.revenueValue}>{auditData.revenueOpportunity.optimizedEstimate}</Text>
            </View>
            <View style={styles.revenueBox}>
          ...
          <Text style={styles.paragraph}>{auditData.revenueOpportunity.breakdown}</Text>
        </View>
      </Page>
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.section}>
          <View style={styles.marketRow}>
            ...
            <Text style={styles.marketValue}>{auditData.marketAnalysis.location}</Text>
          </View>
          <View style={styles.marketRow}>
            <Text style={styles.marketLabel}>Local Competition:</Text>
            <Text style={styles.marketValue}>{auditData.marketAnalysis.localCompetition}</Text>
          </View>
          <View style={styles.marketRow}>
            <Text style={styles.marketLabel}>Average Session Rate:</Text>
            <Text style={styles.marketValue}>{auditData.marketAnalysis.averageSessionRate}</Text>
          </View>

          <Text style={styles.subheading}>Demand Indicators</Text>
          {auditData.marketAnalysis.demandIndicators.map((indicator, index) => (
            <Text key={index} style={styles.bulletPoint}>• {indicator}</Text>
          ))}
      ...
      <Page size="A4" style={styles.page}>
        ...
        {auditData.competitorAnalysis.topCompetitors.map((competitor, index) => (
          <View key={index} style={styles.competitorCard}>
            ...
            {competitor.weaknesses.map((weakness, i) => (
              <Text key={i} style={styles.bulletPoint}>• {weakness}</Text>
            ))}
        ...
        {auditData.competitorAnalysis.competitiveAdvantages.map((advantage, index) => (
          <Text key={index} style={styles.bulletPoint}>• {advantage}</Text>
        ))}

        <Text style={styles.subheading}>Gaps to Fill</Text>
        {auditData.competitorAnalysis.gapsToFill.map((gap, index) => (
          <Text key={index} style={styles.bulletPoint}>• {gap}</Text>
        ))}
      </Page>
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.beforeAfterBox}>
            ...
            <Text style={styles.beforeText}>{auditData.optimizationPreview.headline.before}</Text>
          </View>
          <View style={styles.beforeAfterBox}>
            <Text style={styles.afterLabel}>AFTER:</Text>
            <Text style={styles.afterText}>{auditData.optimizationPreview.headline.after}</Text>
          </View>
          <Text style={styles.reasoning}>{auditData.optimizationPreview.headline.reasoning}</Text>
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.beforeAfterBox}>
            <Text style={styles.beforeLabel}>BEFORE:</Text>
            <Text style={styles.beforeText}>{auditData.optimizationPreview.aboutMeOpening.before}</Text>
          </View>
          <View style={styles.beforeAfterBox}>
            <Text style={styles.afterLabel}>AFTER:</Text>
            <Text style={styles.afterText}>{auditData.optimizationPreview.aboutMeOpening.after}</Text>
          </View>
          <Text style={styles.reasoning}>{auditData.optimizationPreview.aboutMeOpening.reasoning}</Text>
        </View>
      </Page>
      ...
      <Page size="A4" style={styles.page}>
        ...
        {Object.entries(auditData.implementationRoadmap).map(([week, data]) => (
          <View key={week} style={styles.roadmapWeek}>
            <Text style={styles.weekTitle}>{formatWeekName(week)} - {data.focus}</Text>
            <Text style={styles.weekTime}>Estimated Time: {data.estimatedTime}</Text>
      ...
      <Page size="A4" style={styles.page}>
        <View style={styles.ctaPage}>
          ...
          <Text style={styles.ctaParagraph}>
            This audit shows you exactly what needs to be fixed and how to do it.
            But we understand that implementing these changes takes time you might not have.
          </Text>
          <Text style={styles.ctaSubtitle}>Done-For-You Optimization Service</Text>
          <Text style={styles.ctaParagraph}>
            Let our team of Psychology Today experts handle everything for you:
          </Text>
          <Text style={styles.ctaBullet}>✓ Complete profile rewrite using proven frameworks</Text>
          <Text style={styles.ctaBullet}>✓ Professional photo guidance and optimization</Text>
          <Text style={styles.ctaBullet}>✓ SEO optimization for maximum visibility</Text>
          <Text style={styles.ctaBullet}>✓ Conversion-focused headline and specialties</Text>
          <Text style={styles.ctaBullet}>✓ Before/after tracking and analytics</Text>
